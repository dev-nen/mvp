...
begin
  ...
  select
    ...
    case
      when length(trim(coalesce(activities.description, ''))) <= 180 then
        trim(coalesce(activities.description, ''))
      else
        trim(left(coalesce(activities.description, ''), 177)) || '...'
    end as short_description,
    coalesce(activities.description_format, 'plain') as description_format,
    ...
    drafts.source_type as draft_source_type,
    drafts.source_label as draft_source_label,
    drafts.review_status as draft_review_status
...
comment on function public.list_internal_admin_activities(text) is
  'Internal admin read model for all non-deleted activities, including unpublished rows.';

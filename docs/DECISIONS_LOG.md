...
| 2026-04-21 | Public `/pvi` is a transitional placeholder and should be removed once the private internal reporting path is validated | Avoid keeping a purposeless frontend route alive after the browser-side dashboard line was intentionally retired | The current branch still exposes `/pvi` as a safe placeholder, but future work should remove it instead of expanding it |

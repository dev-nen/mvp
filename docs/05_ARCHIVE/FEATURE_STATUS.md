...
| Internal Draft Inbox | Partial | Internal routes, guard, services, review form, approval path, internal manual draft creation, controlled Markdown descriptions, and cover image draft upload artifacts are implemented in repo | Still depends on applying the SQL, granting internal access rows, storage policy rollout, and running live validation in Supabase |
...
| Vercel Web Analytics | Partial | `@vercel/analytics` is installed and `<Analytics />` is mounted once at the app root in `src/App.jsx` | Requires dashboard enablement and live collection verification; internal route exclusion remains a product/ops decision |

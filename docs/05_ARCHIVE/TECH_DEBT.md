...
| Draft publish boundary | Draft approval and approved-activity lifecycle currently manage only `activities` and intentionally skip `activity_contact_options` | Internal draft creation is broader, but still not fully complete for contact publication |
| Deprecated catalog excerpt | `short_description` remains in `catalog_activities_read` only as deprecated compatibility output | Future cleanup should remove it after contracts and deploys no longer depend on it |
| Source creation | Draft Inbox can create manual internal drafts, but still does not ingest external sources, OCR, PDFs, emails, CSVs, or connectors | Scout Manual/source intake remains a later phase |

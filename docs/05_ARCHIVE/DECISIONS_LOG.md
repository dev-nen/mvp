...
| 2026-04-17 | PVI remains Supabase-backed only until `activity_events` is real and readable | Avoid inventing a temporary browser-local analytics source that would blur the product boundary | Browser-side analytics dashboard reads stay retired while reporting remains server-side |
| 2026-04-21 | Public `/pvi` was only a transitional placeholder and should be removed once the private internal reporting path exists | Avoid keeping a purposeless frontend route alive after the browser-side dashboard line was intentionally retired | `main` no longer exposes `/pvi`; reporting remains only through `api/internal/pvi` |
...
| 2026-05-12 | Onboarding should use DIR3-coded municipalities, not catalog-derived city choices | Keep profile municipality selection independent from current catalog coverage | `municipality_choices_read` is the preferred onboarding source; Les Roquetes/Roquetas remains a temporary exception mapped to Sant Pere de Ribes |
| 2026-05-14 | Public contact reads should use a safe read view | Avoid exposing raw contact rows for inactive/deleted/non-public activities | Frontend reads `activity_contact_options_read`; raw `activity_contact_options` public reads are revoked by hardening SQL |
| 2026-05-14 | Static UI i18n is controlled and intentionally limited | Support ES/CA/EN UI without pretending dynamic catalog content is translated | UI dictionaries live under `src/i18n`; dynamic activity, center, city and contact content remains source-language |
| 2026-05-14 | Technical review docs need a single entry point | Reduce docs drift for human and AI-assisted review | `docs/README.md` is the canonical docs entry point; `docs/DOCS_INDEX.md` remains compatibility redirect |
